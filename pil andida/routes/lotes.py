...
def listar():
    ...
    productos = execute_query(
        """
        SELECT id, nombre_comercial, presentacion
        FROM productos
...
def guardar():
    ...
    planta = execute_query(
        "SELECT id FROM plantas WHERE nombre = %s",
        (planta_nombre,),
        fetch_one=True
    )
    ...
    if data['calidad_resultado'] == 'Aprobado':
        ...
        bodega = execute_query(
            ...
            FROM bodegas
            WHERE planta_id = %s
            AND tipo_bodega = 'Producto Terminado'
            LIMIT 1
            """,
            (planta['id'],),
            fetch_one=True
...
def editar(id):
    ...
    productos = execute_query(
        "SELECT id, nombre_comercial FROM productos",
        fetch_all=True
    )

    plantas = execute_query(
        "SELECT id, nombre FROM plantas",
        fetch_all=True
    )
...
def eliminar(id):
    ...
    movs = execute_query(
        "SELECT COUNT(*) as total FROM movimientos WHERE lote_id = %s",
        (id,),
        fetch_one=True
    )
