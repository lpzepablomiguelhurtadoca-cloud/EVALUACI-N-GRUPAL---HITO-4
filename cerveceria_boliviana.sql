...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-06-2026 a las 23:19:09
...
--
-- Base de datos: `cerveceria_boliviana`
--
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `mover_lote_caducado` ()   BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE l_id INT;
    DECLARE cur CURSOR FOR SELECT id FROM lotes WHERE fecha_vencimiento < CURDATE() AND control_calidad = 'Aprobado';
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    OPEN cur;
    read_loop: LOOP
        FETCH cur INTO l_id;
        IF done THEN LEAVE read_loop; END IF;
        UPDATE lotes SET control_calidad = 'Rechazado' WHERE id = l_id;
        INSERT INTO movimientos (tipo_movimiento, lote_id, cantidad, usuario_responsable, observaciones)
        VALUES ('SALIDA', l_id, (SELECT cantidad FROM stock WHERE lote_id = l_id LIMIT 1), 'sistema', 'Lote caducado eliminado');
        UPDATE stock SET cantidad = 0 WHERE lote_id = l_id;
    END LOOP;
    CLOSE cur;
END$$
...
INSERT INTO `logs_auditoria` (`id`, `usuario`, `accion`, `tabla_afectada`, `registro_id`, `detalles`, `ip_address`, `fecha`) VALUES
(1, 'admin', 'LOGIN', NULL, NULL, 'Inicio de sesión exitoso', '127.0.0.1', '2026-06-05 21:10:43'),
(2, 'admin', 'LOGIN', NULL, NULL, 'Inicio de sesión exitoso', '127.0.0.1', '2026-06-05 21:11:06'),
(3, 'admin', 'LOGIN', NULL, NULL, 'Inicio de sesión exitoso', '127.0.0.1', '2026-06-05 21:14:58');
...
--
DELIMITER $$
CREATE TRIGGER `after_movimiento_log` AFTER INSERT ON `movimientos` FOR EACH ROW BEGIN
    INSERT INTO logs_auditoria (usuario, accion, tabla_afectada, registro_id, detalles)
    VALUES (NEW.usuario_responsable, CONCAT('MOVIMIENTO_', NEW.tipo_movimiento), 'movimientos', NEW.id, 
            CONCAT('Cantidad: ', NEW.cantidad, ' Lote: ', NEW.lote_id));
END
$$
DELIMITER ;
...
CREATE TABLE `usuarios_sistema` (
  ...
  `rol` enum('admin','gerente','distribuidor') NOT NULL,
...
INSERT INTO `usuarios_sistema` (`id`, `username`, `password_hash`, `rol`, `nombre_completo`, `activo`, `created_at`) VALUES
(2, 'admin2', '123', 'admin', 'Administrador del Sistema', 1, '2026-06-05 20:49:45'),
(3, 'admin', 'scrypt:32768:8:1$LTxN140BeRRlt84J$8bc74f738676f4aa9099712b2264b57142dc76a77014303a631287034500a1497d91b0cdfa7d14f06af0aa5231f1fec553cfe06f93a623859e9a1c394a1dd46a', 'admin', 'Administrador', 1, '2026-06-05 20:54:15');
